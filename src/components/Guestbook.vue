<template>
    <div id="guestbook" class="main_Block">
        <div class="content_Box">
            <div style="z-index: 10002" class="modal-mask" v-if="m_Modal"></div>
            <div class="message_Add" @click="message_Modal"><i class="fas fa-plus-circle"></i>新增留言</div>
            <div class="message message_Lists">
                <div class="m_Lists">

                    <div class="m_List" v-for="m in message_Lists">
                        <div class="m_Time">
                            {{m.CreateTime.substr(0,10)}}<br>
                            {{m.CreateTime.substr(11,5)}}
                        </div>
                        <div class="m_Main pic">
                            <img v-if="m.Picture" :src="'http://www.henrychang.tw/images/work/'+m.Picture" alt="" onerror="this.src='data:image/jpeg;base64,'">
                            <img v-else src="../assets/default.png" alt="">
                        </div>
                        <div class="m_Main">
                            <h3>{{m.UserName}}</h3>
                            <p>{{m.Email}}</p>
                        </div>
                        <p class="m_Content">
                            <span class="marks before">“</span>{{m.Content}}<span class="marks after">”</span></p>
                        <p class="m_Title">{{m.Title}}</p>
                        <div class="m_Time-phone">
                            {{m.CreateTime.substr(0,10)}} {{m.CreateTime.substr(11,5)}}
                        </div>
                    </div>
                </div>
            </div>
            <div class="message message_Form">

                    <h2 class="title">新增留言</h2>
                    <div class="form_Item">
                        <label for="msg_Name" class="required">暱稱</label>
                        <input id="msg_Name" type="text" v-model="new_Username" required>
                    </div>
                    <div class="form_Item file">
                        <input multiple id="file_Img" type="file" @change="fileChange" ref="inputer">
                        <label name="file_Img" for="file_Img">上傳頭像</label>
                        <div class="img_Box">

                            <img src="" alt="" id="imgUrl">
                        </div>
                    </div>
                    <div class="form_Item">
                        <label for="msg_Email" class="required">信箱</label>
                        <input id="msg_Email" type="text" v-model="new_Email" required>
                    </div>
                    <div class="form_Item">
                        <label for="msg_Title" class="required">標題</label>
                        <input id="msg_Title" type="text" v-model="new_Title" required>
                    </div>
                    <div class="form_Item">
                        <label for="msg_Content" class="required">內容</label>
                        <textarea name="" id="msg_Content" cols="30" rows="10" v-model="new_Content" required></textarea>
                    </div>
                    <div class="form_Button">
                        <button class="btn grey" @click="message_Cancel()">取消</button>
                        <button class="btn" @click="message_Post()">送出</button>
                    </div>
            </div>
        </div>
        <div class="message_Modal" v-if="m_Modal">
            <div @click="close_Modal">
                <i class="fas fa-times cancel"></i>
            </div>
            <div class="m_Modal-box">

                <div class="form_Item">
                    <label for="msg_Name" class="required">暱稱</label>
                    <input id="msg_Name" type="text" v-model="new_Username" required>
                </div>
                <div class="form_Item file">
                    <input multiple id="file_Img2" type="file" @change="fileChange" ref="inputer">
                    <label name="file_Img" for="file_Img2">上傳頭像</label>
                    <div class="img_Box">

                        <img src="" alt="" id="imgUrl2">
                    </div>
                </div>
                <div class="form_Item">
                    <label for="msg_Email" class="required">信箱</label>
                    <input id="msg_Email" type="text" v-model="new_Email" required>
                </div>
                <div class="form_Item">
                    <label for="msg_Title" class="required">標題</label>
                    <input id="msg_Title" type="text" v-model="new_Title" required>
                </div>
                <div class="form_Item">
                    <label for="msg_Content" class="required">內容</label>
                    <textarea name="" id="msg_Content" cols="30" rows="10" v-model="new_Content" required></textarea>
                </div>
                <div class="form_Button">
                    <button class="btn grey" @click="message_Cancel()">取消</button>
                    <button class="btn" @click="message_Post()">送出</button>
                </div>
            </div>
        </div>

    </div>
</template>

<script>
    export default {
        data(){
            return{
                message_Lists:[
                ],
                m_Modal: false,
                new_Username: "",
                new_Email: "",
                new_Title: "",
                new_Content: "",
                form_Img: "",
                formData: new FormData(),
                files: '', /* 上傳圖片file */
                imgUrl: '' /* 上傳圖片url */
            }
        },
        mounted(){
            axios.post('/data/APITest/GetMessageBoard')
            .then((resp) => {
                this.message_Lists = JSON.parse(resp.data.content)
                console.log(this.message_Lists)
            });
        },
        methods:{
            imgPreview(file, id){

                let self = this;
                // 看支持不支持FileReader
                if (!file || !window.FileReader) return;
        
                if (/^image/.test(file.type)) {
                    // 創建一個reader
                    let reader = new FileReader();
                    // 將圖片將轉成 base64 格式
                    reader.readAsDataURL(file);
                    // 讀取成功後的回調
                    reader.onloadend = function () {
                        if( id == 'file_Img'){
                            document.getElementById('imgUrl').src = this.result;
                        }else if( id == 'file_Img2'){
                            document.getElementById('imgUrl2').src = this.result;
                        }
                    }
                    
                }
            },
            fileChange(e) {
                // 獲取input DOM
                let inputDOM = this.$refs.inputer;
                this.files = e.target.files[0];

                this.formData.append('file', this.files)
        
                // 限制圖片大小
                let size = Math.floor(this.files.size  / 1024);
                if (size > 100) {
                    return false
                }

                this.imgPreview(this.files,inputDOM.id);
            },
            message_Cancel(){
                this.new_Username = '';
                this.new_Email = '';
                this.new_Title = '';
                this.new_Content = '';
                document.getElementById('imgUrl').src = '';
                document.getElementById('imgUrl2').src = '';
                
                alert('已取消留言')
                this.m_Modal = false
            },
            message_Post(){
                let formData = new FormData();
                formData.append('username', this.new_Username)
                formData.append('email', this.new_Email)
                formData.append('title', this.new_Title)
                formData.append('content', this.new_Content)
                formData.append('file', this.files)

                axios.post('/data/APITest/AddMessageBoard',formData
                ).then((resp) => {
                    if(resp.data.result == '1'){
                        alert(resp.data.content);
                        // location.reload();
                        this.m_Modal = false;
                        this.message_Refesh();

                        this.new_Username = '';
                        this.new_Email = '';
                        this.new_Title = '';
                        this.new_Content = '';
                        document.getElementById('imgUrl').src = '';
                        document.getElementById('imgUrl2').src = '';

                    }else{
                        alert('留言失敗，請重新輸入內容')

                    }

                });

            },
            message_Refesh(){
                axios.post('/data/APITest/GetMessageBoard')
                .then((resp) => {
                    this.message_Lists = JSON.parse(resp.data.content)
                    console.log('更新成功',this.message_Lists)
                });
            },
            message_Modal(){
                this.m_Modal = true
            },
            close_Modal(){
                this.m_Modal = false
                this.new_Username = '';
                this.new_Email = '';
                this.new_Title = '';
                this.new_Content = '';
                document.getElementById('imgUrl').src = '';
                document.getElementById('imgUrl2').src = '';
            },

        }
    }
</script>